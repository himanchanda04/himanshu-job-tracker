...
import {
  ...
  extractKeywordsFromResume, detectGhostJob,
...
async function runScout(userId, triggeredBy = 'manual') {
  ...
  try {
    ...
    // 4 — Score (AI or keyword fallback)
    ...
    // 5 — Threshold + ghost detection
    const existing = await pool.query(
      `SELECT company, title FROM scout_results WHERE user_id=$1`, [userId]
    );
    const matched = scoredJobs
      .filter(j => !j.score || j.score >= user.min_score)
      .map(j => ({ ...j, is_ghost_job: detectGhostJob(j, existing.rows) }))
      .sort((a, b) => (b.score || 0) - (a.score || 0));

    console.log(`[Scout] Matched (≥${user.min_score}): ${matched.length}`);
    ...
    // 7 — Auto-expire seen jobs older than 30 days
    await pool.query(
    ...
    await pool.query(
      `UPDATE scout_runs SET completed_at=NOW(), status='completed',
       ...
       WHERE id=$6`,
      [allJobs.length, matched.length, sourcesUsed, sourcesFailed, totalClaudeCost, runId]
    );

    // 9 — Email if new jobs + opted in
    if (newCount > 0 && user.email_notify) {
      sendScoutDigest(user.email, user.name, matched.slice(0, 10), { sourcesUsed })
        .catch(e => console.warn('[Scout] Email failed (non-fatal):', e.message));
    }

    return { runId, jobsFound: newCount, totalMatched: matched.length, sourcesUsed, sourcesFailed };
...
runScoutMigration().catch(console.error);
