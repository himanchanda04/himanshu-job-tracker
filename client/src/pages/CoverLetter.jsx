...
import { Mail, Sparkles, Copy, Check, RotateCcw, AlertCircle, Upload, Download, ChevronDown } from 'lucide-react';
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <div className="bg-white rounded-xl shadow-card p-5">
          <Textarea label="Your Resume" placeholder="Paste your resume or upload PDF/DOCX…" value={resume} onChange={setResume} />
        </div>
        <div className="bg-white rounded-xl shadow-card p-5">
          ...
          <div className="flex items-center justify-between">
            ...
            {output && !loading && (
              <div className="flex items-center gap-2">
                ...
                <DownloadMenu text={output} filename="cover-letter" />
              </div>
            )}
          </div>
          <pre className="whitespace-pre-wrap text-sm text-slate-700 leading-relaxed bg-slate-50 rounded-lg p-4 max-h-[500px] overflow-y-auto">{output || ' '}</pre>
        </div>
      )}
    </div>
  );
