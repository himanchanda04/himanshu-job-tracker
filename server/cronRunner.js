...
import { passesQualityGates, detectGhostJob, detectFakeJob } from './services/qualityFilter.js';
...
async function main() {
  ...
  const { rows: users } = await pool.query(`
    SELECT u.id, u.name, u.email, u.original_resume,
  ...
  if (!users.length) { console.log('[CronRunner] No active users.'); process.exit(0); }
  ...
  for (const user of users) {
    try {
      ...
      if (!user.original_resume) { console.log(`[CronRunner] No resume — skipping.`); continue; }

      const { rows: [run] } = await pool.query(
        `INSERT INTO scout_runs (user_id, triggered_by, status, started_at)
         VALUES ($1, 'cron', 'running', NOW()) RETURNING id`, [user.id]
      );
      const runId = run.id;

      const { allJobs, sourcesUsed, sourcesFailed } = await fetchAllSources(
        user.target_title || 'Marketing', user.industry || '', user.id
      );
      ...
      const filtered = unique.filter(j => passesQualityGates(j).pass);
      console.log(`[CronRunner] After quality filter: ${filtered.length} jobs`);

      if (!filtered.length) {
        await pool.query(`UPDATE scout_runs SET status='completed', completed_at=NOW(), jobs_fetched=$1, jobs_matched=0, sources_used=$2, sources_failed=$3 WHERE id=$4`,
          [allJobs.length, sourcesUsed, sourcesFailed, runId]);
        console.log(`[CronRunner] No jobs passed quality filter.`);
        continue;
      }

      const { scoredJobs, totalClaudeCost } = await scoreJobs(filtered, user.original_resume, user.target_title, user.industry, user.id, runId);

      const { rows: existing } = await pool.query(`SELECT company, title FROM scout_results WHERE user_id=$1`, [user.id]);
      ...
      for (const job of annotated) {
        const bucket = primaryIds.has(job.job_hash) ? 'primary' : secondaryIds.has(job.job_hash) ? 'secondary' : null;
        const ins = await pool.query(
          `INSERT INTO scout_results (user_id, job_hash, title, company, location, url, description, sources, posted_at, score, category, matched_keywords, missing_keywords, is_ghost_job, scoring_mode, status, last_seen_at, noc_code, noc_teer, pr_eligible, pr_tier, title_match, job_category, fake_signal_count, digest_bucket)
...
main().catch(err => { console.error('[CronRunner] Fatal error:', err); process.exit(1); });
